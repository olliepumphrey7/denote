var DenoteEditor = (() => {
  ...
  __export(prosemirror_editor_exports, {
    ...
    documentFromHTML: () => documentFromHTML,
    documentFromModelJSON: () => documentFromModelJSON,
    headingTriggers: () => headingTriggers,
    makeInputRules: () => makeInputRules,
    makeKeymapBindings: () => makeKeymapBindings,
    ...
    updateChildren(view2, pos) {
      ...
      iterDeco(this.node, this.innerDeco, (widget, i, insideNode) => {
        if (widget.spec.marks)
          updater.syncToMarks(widget.spec.marks, inline, view2, i);
        else if (widget.type.side >= 0 && !insideNode)
          updater.syncToMarks(i == this.node.childCount ? Mark.none : this.node.child(i).marks, inline, view2, i);
        updater.placeWidget(widget, view2, off);
      }, (child, outerDeco, innerDeco, i) => {
        updater.syncToMarks(child.marks, inline, view2, i);
        let compIndex;
  ...
  function selectCursorWrapper(view2) {
    ...
    if (!img && !view2.state.selection.visible && ie2 && ie_version <= 11) {
      node.disabled = true;
      node.disabled = false;
    }
  }
  ...
  function parseFromClipboard(view2, text, html, plainText2, $context) {
    let inCode = $context.parent.type.spec.code;
    let dom, slice2;
    if (!html && !text)
      return null;
    let asText = !!text && (plainText2 || inCode || !html);
    if (asText) {
      view2.someProp("transformPastedText", (f) => {
        text = f(text, inCode || plainText2, view2);
      });
      if (inCode) {
      ...
      let parsed = view2.someProp("clipboardTextParser", (f) => f(text, $context, plainText2, view2));
      if (parsed) {
        slice2 = parsed;
      } else {
  ...
  editHandlers.keypress = (view2, _event) => {
    let event = _event;
    if (inOrNearComposition(view2, event) || !event.charCode || event.ctrlKey && !event.altKey || mac4 && event.metaKey)
      return;
  ...
  editHandlers.compositionstart = editHandlers.compositionupdate = (view2) => {
    if (!view2.composing) {
      ...
      if (state.selection instanceof TextSelection && (state.storedMarks || !$pos.textOffset && $pos.parentOffset && $pos.nodeBefore.marks.some((m) => m.type.spec.inclusive === false) || chrome && windows2 && selectionBeforeUneditable(view2))) {
  ...
  function capturePaste(view2, event) {
    if (!view2.dom.parentNode)
      return;
    let plainText2 = view2.input.shiftKey || view2.state.selection.$from.parent.type.spec.code;
    let target = view2.dom.parentNode.appendChild(document.createElement(plainText2 ? "textarea" : "div"));
    if (!plainText2)
  ...
  var DOMObserver = class {
    ...
    flush() {
      ...
      if (added.some((n) => n.nodeName == "BR") && (view2.input.lastKeyCode == 8 || view2.input.lastKeyCode == 46)) {
  ...
  function readDOMChange(view2, from2, to, typeOver, addedNodes) {
    ...
    let mkTr = (base2) => {
      ...
      if (parse.sel) {
        ...
        if (sel2 && !(chrome && view2.composing && sel2.empty && (change.start != change.endB || view2.input.lastChromeDelete < Date.now() - 100) && (sel2.head == chFrom || sel2.head == tr.mapping.map(chTo) - 1) || ie2 && sel2.empty && sel2.head == chFrom))
  ...
  function buildNodeViews(view2) {
    ...
    view2.someProp("nodeViews", add2);
    view2.someProp("markViews", add2);
    return result;
  }
  ...
  function tableNodeTypes(schema3) {
    let result = schema3.cached.tableNodeTypes;
    if (!result) {
      result = schema3.cached.tableNodeTypes = {};
      for (const name in schema3.nodes) {
        const type = schema3.nodes[name], role = type.spec.tableRole;
  ...
  function pastedCells(slice2) {
    ...
    const schema3 = first.type.schema, rows = [];
    if (role == "row") for (let i = 0; i < content.childCount; i++) {
      let cells = content.child(i).content;
      const left = i ? 0 : Math.max(0, openStart - 1);
      const right = i < content.childCount - 1 ? 0 : Math.max(0, openEnd - 1);
      if (left || right) cells = fitSlice(tableNodeTypes(schema3).row, new Slice(cells, left, right)).content;
      rows.push(cells);
    }
    else if (role == "cell" || role == "header_cell") rows.push(openStart || openEnd ? fitSlice(tableNodeTypes(schema3).row, new Slice(content, openStart, openEnd)).content : content);
    else return null;
    return ensureRectangular(schema3, rows);
  }
  function ensureRectangular(schema3, rows) {
    const widths = [];
    ...
    for (let r = 0; r < widths.length; r++) {
      ...
      if (widths[r] < width) {
        const empty2 = tableNodeTypes(schema3).cell.createAndFill();
        const cells = [];
  ...
  function growTable(tr, map2, table, start, width, height, mapFrom) {
    const schema3 = tr.doc.type.schema;
    const types = tableNodeTypes(schema3);
    let empty2;
    let emptyHead;
  ...
  function arrow(axis, dir) {
    return (state, dispatch, view2) => {
      ...
      const end = atEndOfCell(view2, axis, dir);
  ...
  function handleTripleClick2(view2, pos) {
    const doc3 = view2.state.doc, $cell = cellAround(doc3.resolve(pos));
  ...
  function handlePaste(view2, _, slice2) {
    ...
    if (sel instanceof CellSelection) {
      if (!cells) cells = {
        width: 1,
        height: 1,
        rows: [Fragment.from(fitSlice(tableNodeTypes(view2.state.schema).cell, slice2))]
      };
      const table = sel.$anchorCell.node(-1);
      const start = sel.$anchorCell.start(-1);
      const rect = TableMap.get(table).rectBetween(sel.$anchorCell.pos - start, sel.$headCell.pos - start);
      cells = clipCells(cells, rect.right - rect.left, rect.bottom - rect.top);
      insertCells(view2.state, view2.dispatch, start, rect, cells);
      return true;
    } else if (cells) {
      const $cell = selectionCell(view2.state);
      const start = $cell.start(-1);
      insertCells(view2.state, view2.dispatch, start, TableMap.get($cell.node(-1)).findCell($cell.pos - start), cells);
      return true;
    } else return false;
  }
  function handleMouseDown$1(view2, startEvent) {
    var _cellUnderMouse;
    if (startEvent.button != 0) return;
    if (startEvent.ctrlKey || startEvent.metaKey) return;
    const startDOMCell = domInCell(view2, startEvent.target);
    let $anchor;
    if (startEvent.shiftKey && view2.state.selection instanceof CellSelection) {
      setCellSelection(view2.state.selection.$anchorCell, startEvent);
      startEvent.preventDefault();
    } else if (startEvent.shiftKey && startDOMCell && ($anchor = cellAround(view2.state.selection.$anchor)) != null && ((_cellUnderMouse = cellUnderMouse(view2, startEvent)) === null || _cellUnderMouse === void 0 ? void 0 : _cellUnderMouse.pos) != $anchor.pos) {
      setCellSelection($anchor, startEvent);
      startEvent.preventDefault();
    } else if (!startDOMCell) return;
    function setCellSelection($anchor$1, event) {
      let $head = cellUnderMouse(view2, event);
      const starting = tableEditingKey.getState(view2.state) == null;
      if (!$head || !inSameTable($anchor$1, $head)) if (starting) $head = $anchor$1;
      else return;
      const selection = new CellSelection($anchor$1, $head);
      if (starting || !view2.state.selection.eq(selection)) {
        const tr = view2.state.tr.setSelection(selection);
        if (starting) tr.setMeta(tableEditingKey, $anchor$1.pos);
        view2.dispatch(tr);
      }
    }
    function stop() {
      view2.root.removeEventListener("mouseup", stop);
      view2.root.removeEventListener("dragstart", stop);
      view2.root.removeEventListener("mousemove", move);
      if (tableEditingKey.getState(view2.state) != null) view2.dispatch(view2.state.tr.setMeta(tableEditingKey, -1));
    }
    function move(_event) {
      const event = _event;
      const anchor = tableEditingKey.getState(view2.state);
      let $anchor$1;
      if (anchor != null) $anchor$1 = view2.state.doc.resolve(anchor);
      else if (domInCell(view2, event.target) != startDOMCell) {
        $anchor$1 = cellUnderMouse(view2, startEvent);
        if (!$anchor$1) return stop();
      }
      if ($anchor$1) setCellSelection($anchor$1, event);
    }
    view2.root.addEventListener("mouseup", stop);
    view2.root.addEventListener("dragstart", stop);
    view2.root.addEventListener("mousemove", move);
  }
  ...
  function domInCell(view2, dom) {
    for (; dom && dom != view2.dom; dom = dom.parentNode) if (dom.nodeName == "TD" || dom.nodeName == "TH") return dom;
    return null;
  }
  function cellUnderMouse(view2, event) {
    const mousePos = view2.posAtCoords({
      left: event.clientX,
      top: event.clientY
    });
    if (!mousePos) return null;
    let { inside, pos } = mousePos;
    return inside >= 0 && cellAround(view2.state.doc.resolve(inside)) || cellAround(view2.state.doc.resolve(pos));
  }
  var TableView = class {
    ...
    const plugin = new Plugin({
      ...
      state: {
        init(_, state) {
          ...
          if (View && nodeViews) nodeViews[tableName] = (node, view2) => {
            return new View(node, defaultCellMinWidth, view2);
          };
      ...
      props: {
        ...
        handleDOMEvents: {
          mousemove: (view2, event) => {
            handleMouseMove(view2, event, handleWidth, lastColumnResizable);
          },
          mouseleave: (view2) => {
            handleMouseLeave(view2);
          },
          mousedown: (view2, event) => {
            handleMouseDown(view2, event, cellMinWidth, defaultCellMinWidth);
          }
        },
        decorations: (state) => {
          const pluginState = columnResizingPluginKey.getState(state);
  ...
  function handleMouseMove(view2, event, handleWidth, lastColumnResizable) {
  ...
  function handleMouseLeave(view2) {
    if (!view2.editable) return;
    const pluginState = columnResizingPluginKey.getState(view2.state);
    if (pluginState && pluginState.activeHandle > -1 && !pluginState.dragging) updateHandle(view2, -1);
  }
  function handleMouseDown(view2, event, cellMinWidth, defaultCellMinWidth) {
    ...
    function finish(event$1) {
      ...
      const pluginState$1 = columnResizingPluginKey.getState(view2.state);
      if (pluginState$1 === null || pluginState$1 === void 0 ? void 0 : pluginState$1.dragging) {
        updateColumnWidth(view2, pluginState$1.activeHandle, draggedWidth(pluginState$1.dragging, event$1, cellMinWidth));
        view2.dispatch(view2.state.tr.setMeta(columnResizingPluginKey, { setDragging: null }));
      }
    }
    function move(event$1) {
      ...
      if (pluginState$1.dragging) {
        ...
        displayColumnWidth(view2, pluginState$1.activeHandle, dragged, defaultCellMinWidth);
      }
    }
    displayColumnWidth(view2, pluginState.activeHandle, width, defaultCellMinWidth);
    win.addEventListener("mouseup", finish);
    win.addEventListener("mousemove", move);
    event.preventDefault();
    return true;
  }
  function currentColWidth(view2, cellPos, { colspan, colwidth }) {
    const width = colwidth && colwidth[colwidth.length - 1];
    if (width) return width;
    const dom = view2.domAtPos(cellPos);
  ...
  function edgeCell(view2, event, side, handleWidth) {
    const offset = side == "right" ? -handleWidth : handleWidth;
    const found2 = view2.posAtCoords({
      left: event.clientX + offset,
      top: event.clientY
    });
    if (!found2) return -1;
    const { pos } = found2;
    const $cell = cellAround(view2.state.doc.resolve(pos));
    if (!$cell) return -1;
  ...
  function tableEditing({ allowTableNodeSelection = false } = {}) {
    return new Plugin({
      ...
      props: {
        ...
        createSelectionBetween(view2) {
          return tableEditingKey.getState(view2.state) != null ? view2.state.selection : null;
        },
  ...
  function documentFromHTML(html) {
    ...
    return DOMParser.fromSchema(schema2).parse(container);
  }
  function documentFromModelJSON(modelJSON, fallbackHTML) {
    if (modelJSON) {
      try {
        return schema2.nodeFromJSON(JSON.parse(modelJSON));
      } catch {
  ...
  function makeInputRules() {
    return inputRules({
      rules: [
        ...
        wrappingInputRule(/^(\d+)\.\s$/, schema2.nodes.ordered_list, (match) => ({ order: +match[1] }), (match, node) => node.childCount + node.attrs.order === +match[1]),
        ...headingTriggers.map((trigger) => textblockTypeInputRule(trigger.pattern, schema2.nodes.heading, { level: trigger.level })),
        textblockTypeInputRule(/^>\s$/, schema2.nodes.blockquote)
      ]
    });
  }
  function deleteSelectedTable(state, dispatch) {
    const selection = state.selection;
    if (selection instanceof CellSelection || selection instanceof NodeSelection && selection.node.type === schema2.nodes.table) {
  ...
  function normalizeDocumentTransaction(state) {
    let transaction = fixTables(state);
    let doc3 = transaction ? transaction.doc : state.doc;
    let changed = Boolean(transaction);
    for (let position = doc3.content.size, index = doc3.childCount - 1; index >= 0; index -= 1) {
      const child = doc3.child(index);
      position -= child.nodeSize;
      if (child.type === schema2.nodes.table && tableIsEmpty(child)) {
    ...
    const lastChild = doc3.lastChild;
    if (lastChild && lastChild.type === schema2.nodes.table) {
      if (!transaction) transaction = state.tr;
      transaction.insert(doc3.content.size, schema2.nodes.paragraph.create());
      changed = true;
    }
    return changed ? transaction : null;
  }
  function createEditorState(doc3 = documentFromHTML("<p></p>")) {
  ...
  function makeKeymapBindings() {
    const listItem2 = schema2.nodes.list_item;
    return {
      "Mod-b": toggleMark(schema2.marks.strong),
      "Mod-i": toggleMark(schema2.marks.em),
      "Mod-0": setBlockType2(schema2.nodes.paragraph),
      "Mod-1": setBlockType2(schema2.nodes.heading, { level: 1 }),
      "Mod-2": setBlockType2(schema2.nodes.heading, { level: 2 }),
      "Mod-3": setBlockType2(schema2.nodes.heading, { level: 3 }),
      "Mod-z": undo,
      "Shift-Mod-z": redo,
      "Mod-y": redo,
      "Backspace": deleteSelectedTable,
      "Delete": deleteSelectedTable,
      "Enter": chainCommands(newlineInCode, splitListItem(listItem2), createParagraphNear, liftEmptyBlock, splitBlock),
      "Tab": chainCommands(sinkListItem(listItem2), goToNextCell(1)),
      "Shift-Tab": chainCommands(liftListItem(listItem2), goToNextCell(-1)),
      "Mod-Shift-8": wrapInList(schema2.nodes.bullet_list),
      "Mod-Shift-7": wrapInList(schema2.nodes.ordered_list)
    };
  }
      ...
      new Plugin({
        ...
        props: {
          handlePaste(editorView, event) {
            ...
            const doc3 = documentFromHTML(html);
            const slice2 = doc3.slice(0, doc3.content.size);
            const transaction = editorView.state.tr.replaceSelection(slice2);
  ...
  if (globalThis.window) {
    window.editorSetDocument = (payload) => {
      const doc3 = documentFromModelJSON(payload.modelJSON, payload.html);
      suppressChange = true;
    ...
    window.editorApplyNormalStyle = () => {
      ...
      view.dispatch(view.state.tr.removeMark(from2, to).setBlockType(from2, to, schema2.nodes.paragraph));
      notifyChange();
    };
  ...
  return __toCommonJS(prosemirror_editor_exports);
})();
